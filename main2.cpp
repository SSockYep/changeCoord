...
int main(int argc, char** argv)
{
	...
	for (int i = 0; i < 4; i++)
	{
		GeoCoord newCoord = calcNewCoord(geoCorner[i], params);
		GeoCoord outCoord = calcMapCoord(geoCorner, newCoord, params);
	...
	while (inputFile.eof() == false)
	{
		...
		{
			inputFile >> imgCoords[i] >> geoCoords[i];
			GeoCoord newCoord = calcNewCoord(geoCoords[i], params);
			GeoCoord outCoord = calcMapCoord(geoCorner, newCoord, params);
			outputFile << outCoord.x << ' ' << outCoord.y << ' ';
